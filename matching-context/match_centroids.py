...
from utils_matching import (load_centroids, load_doc_clusters,
                            load_bert, embed_sentence, closest_centroid, weighted_prototype_vector)
...
def process_split(dataset: str, split: str,
    ...
    rows = []
    # avant la boucle
    available_clusters = list(centroids_bank.keys())  # ex: [1,2,4]
    default_cluster = available_clusters[0]  # on prend le premier
    ...
    for doc in tqdm(docs, desc=f"{dataset}/{split}", ncols=80):
        ...
        elif 0 in centroids_bank:  # fallback primaire : cluster 0
            if clu_id not in missing_warned:
                logging.warning(
                    f"[{dataset}/{split}] cluster {clu_id} absent – fallback cluster 0")
                missing_warned.add(clu_id)
            labels, mat = centroids_bank[0]

        else:  # fallback secondaire : premier cluster dispo
            if clu_id not in missing_warned:
                logging.warning(
                    f"[{dataset}/{split}] cluster {clu_id} absent – "
                    f"fallback cluster {default_cluster}")
                missing_warned.add(clu_id)
        ...
        for ann in doc.get("annotations", []):
                for idx, res in enumerate(ann.get("result", [])):
                    ...
                    # nearest_lab, nearest_vec = closest_centroid(vec, mat, labels)
                    if use_weighted:
                        # print("I'm in use_weighted")
                        weighted_vec = weighted_prototype_vector(vec, mat)
                        nearest_lab, _ = closest_centroid(vec, mat, labels)  # label informatif
...
def main():
    ...
    p.add_argument("--centroids_dir", required=True,   # …/<strategy>/<dataset>
                   help="répertoire contenant centroids.jsonl")
    p.add_argument("--emb_type",
                   # choices=("encoder", "decoder", "none"),
                   required=True, help="encoder | decoder | none")
    p.add_argument("--strategy", required=True,
                   help="nom de la stratégie (mean, median, …)")
