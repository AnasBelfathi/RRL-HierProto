...
def pubmed_task(train_batch_size, max_docs, data_folder, task_type="category"):

    # Définir les labels en fonction de task_type
    if task_type == "legal-eval":
        PUBMED_LABELS = ['mask', 'ARG_PETITIONER', 'RATIO', 'RPC', 'ISSUE', 'RLC', 'ANALYSIS', 'PRE_RELIED', 'PREAMBLE', 'NONE', 'ARG_RESPONDENT',
 'STA', 'PRE_NOT_RELIED', 'FAC']
        PUBMED_LABELS_PRES = PUBMED_LABELS

    elif task_type == "scotus-category":
        PUBMED_LABELS = ['mask','Announcing', 'Setting the scene', 'Sources of authority', 'Analysis', 'Resolution']
        PUBMED_LABELS_PRES = PUBMED_LABELS


    elif task_type == "scotus-rhetorical_function":
        PUBMED_LABELS = ['mask', 'Announcing', 'Presenting jurisdiction', 'Describing', 'Quoting', 'Granting certiorari',
                         'Recalling', 'Citing', 'Stating the Court’s reasoning', 'Rejecting arguments/a reasoning',
                         'Giving instructions to competent courts', 'Giving the holding of the Court',
                         'Accepting arguments/a reasoning', 'Evaluating the impact of the decision']
        PUBMED_LABELS_PRES = PUBMED_LABELS

    elif task_type == "scotus-steps":
        PUBMED_LABELS = ['mask', 'Announcing_Announcing', 'Setting the scene_Presenting jurisdiction_Context',
                 'Sources of authority_Describing_Primary source of law',
                 'Setting the scene_Presenting jurisdiction_Legal question(s)',
                 'Setting the scene_Presenting jurisdiction_Adjudicated facts',
                 'Sources of authority_Quoting_Secondary source of law',
                 'Sources of authority_Quoting_Primary source of law',
                 'Sources of authority_Describing_Established practices or cultural norms',
                 'Setting the scene_Presenting jurisdiction_Other procedural events',
                 'Setting the scene_Presenting jurisdiction_Lower court decision',
                 'Sources of authority_Describing_Secondary source of law', 'Setting the scene_Granting certiorari',
                 'Sources of authority_Quoting_SCOTUS decision',
                 'Analysis_Recalling_An adjudicated fact or procedural event_Present case',
                 'Analysis_Recalling_Legal question(s)_Present case', 'Analysis_Recalling_A primary source',
                 'Analysis_Recalling_A secondary source', 'Analysis_Recalling_An established practice or cultural norm',
                 'Sources of authority_Citing_SCOTUS decision', 'Analysis_Stating the Court’s reasoning',
                 'Analysis_Rejecting arguments/a reasoning', 'Analysis_Recalling_An argument_Present case_Respondent',
                 'Analysis_Recalling_An argument_Present case_Petitioner', 'Analysis_Recalling_SCOTUS decision',
                 'Resolution_Giving instructions to competent courts', 'Resolution_Giving the holding of the Court',
                 'Sources of authority_Citing_Primary source of law',
                 'Sources of authority_Citing_Secondary source of law',
                 "Setting the scene_Presenting jurisdiction_Parties' legal claims and arguments",
                 'Analysis_Accepting arguments/a reasoning',
                 'Analysis_Recalling_An adjudicated fact or procedural event_Another case',
                 'Resolution_Evaluating the impact of the decision',
                 'Analysis_Recalling_Legal question(s)_Another case', 'Analysis_Recalling_An argument_Another case',
                 'Analysis_Recalling_An argument_Present case_Dissenting justice(s)']
        PUBMED_LABELS_PRES = PUBMED_LABELS

    elif task_type == "DeepRhole":
        PUBMED_LABELS = ['mask', 'Ruling by Lower Court', 'Facts', 'Ratio of the decision', 'Statute', 'Precedent', 'Ruling by Present Court', 'Argument']
        PUBMED_LABELS_PRES = PUBMED_LABELS

    elif task_type == "biorc":
        PUBMED_LABELS = ['mask', 'BACKGROUND', 'RESULTS', 'CONCLUSIONS', 'METHODS', 'OTHER', 'OBJECTIVE']
        PUBMED_LABELS_PRES = PUBMED_LABELS

    elif task_type == "csabstracts":
        PUBMED_LABELS = ['mask', 'CONCLUSIONS', 'BACKGROUND', 'RESULTS', 'OBJECTIVE', 'METHODS']
        PUBMED_LABELS_PRES = PUBMED_LABELS

    elif task_type == "PubMed_20k_RCT":
        PUBMED_LABELS = ['mask', 'OBJECTIVE', 'BACKGROUND', 'CONCLUSIONS', 'RESULTS', 'METHODS']
        PUBMED_LABELS_PRES = PUBMED_LABELS

    else:
        raise ValueError(
            f"Tâche non reconnue : {task_type}. Choisissez parmi 'category', 'rhetorical_function', 'steps'.")

    return Task(task_type, PUBMED_LABELS, train_batch_size, 1, max_docs, short_name="PMD",
                labels_pres=PUBMED_LABELS_PRES, data_folder=data_folder)
