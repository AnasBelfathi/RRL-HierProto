...
def plot_confusion_matrix(cm, classes,
    ...
    if normalize:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            cm = np.divide(cm.astype('float'), row_sums[:, np.newaxis])
            cm[np.isnan(cm)] = 0  # Replace NaN values with 0
...
def load_best_results(run_path, task, task_name, data_folder):
    '''Loads the relevant metrics for a certain task with the best dev set performance.'''
    results = []
    for best_metrics in load_best_dev_metrics(run_path, task, task_name, data_folder):
...
def load_best_dev_metrics(run_path, task_name, task_name_tmp, data_folder):
    '''Loads the results with the best dev set performance for the given task.'''
    dev_metric = get_task(task_name, task_name_tmp, data_folder).dev_metric
    results = []
...
def get_all_tasks(task_name, data_folder):
    result = []
    result.append(pubmed_task(train_batch_size=-1, max_docs=-1, task_type = task_name, data_folder=data_folder))
...
def get_task(taskname, task_name, data_folder):
    for t in get_all_tasks(task_name, data_folder):
...
def calc_f1(cm):
    ...
    recall = true_pos / (true_pos + false_neg + 1e-10)  # Avoid division by zero
...
def load_confusion_matrix(run_path, task_name, task_name_tmp, data_folder, absolute=True):
    cms = []
    for best_dev_results in load_best_dev_metrics(run_path, task_name, task_name_tmp, data_folder):
...
def eval_and_save_metrics(path, task_name, data_folder):
    '''
    ...
    for task in tasks_in_run:
        task_results = load_best_results(path, task, task_name, data_folder)
    ...
    for t in tasks_in_run:
        task_cm_abs = load_confusion_matrix(path, t,task_name, data_folder , absolute=True)
        task_cm_rel = load_confusion_matrix(path, t,task_name, data_folder ,absolute=False)
        plot_confusion_matrix(
            task_cm_rel,
            get_task(t, task_name, data_folder).labels[1:],
            title=t,
            normalize=True,
            filename=os.path.join(path, f'{t}_cm.pdf'))
        f1s = calc_f1(task_cm_abs)
        for i, f1 in enumerate(f1s):
            label_name = get_task(t, task_name, data_folder).labels[1:][i]
            label_order = get_task(t, task_name, data_folder).labels_pres.index(label_name)
